...
class Docker:
    def __init__(self, names, network=None, options=None, commands=None):
        ...
        self.commands = (
            {name:"" for name in self.names} if (commands is None or not isinstance(options, dict)) else commands
        )
    ...
    def run_containers(self):
        comms = [
            "docker run -d --name " + name + " " + self.options[name] + " " + name + " " + self.commands[name]
            for name in self.names
        ]
